...
в уме. `--json` — граф как JSON (**черновой**: предназначен под внешний **визуализатор структуры для
оператора**, будет дополнен derived-срезами; для ЛЛМ используй текстовый вывод). См. `__dev` Next#4.
```
