...
## Принцип интерфейса: команда = ПРЕДЛОЖЕНИЕ

Флаги — не набор, а их **канонический порядок складывается в предложение**:
**глагол (режим) → объект (цель) → модификатор**.
- `outline --line 15` = «покажи карту у строки 15»;
- `query 15 force` = «дай блок 15 целиком».

Порядок — часть смысла. Команда **читается**, а не парсится — это дешевле для модели (каждое лишнее/
переставленное слово = токены на «зачем оно тут»), и минимизирует словарь (север Vision04).

Следствия:
- **`--force`/`raw` — не «ещё один флаг схемы», а замыкающее НАРЕЧИЕ предложения.** Невидимо, пока не
  нужно; тул подсказывает его в выводе в момент упора в порог (защита масштаба). Синтаксически флаг,
  грамматически — последнее слово фразы. Это НЕ противоречит «мало флагов» — это его форма. (Снимает
  ложную дилемму «force как модификатор vs как флаг».)
- **Полисемия слова допустима, если контекст фиксирует смысл.** `--level` = «потолок» без `--line`,
  «предок цели» с `--line`. Одно знакомое слово, разрешаемое соседями, дешевле трёх точных
  (`--ancestor-level`/`--focus-level`/`--depth-cap`), которые модель должна выучить И выбрать. Оговорка:
  смысл всегда вычисляется грамматикой команды, НЕ угадывается моделью.
- **Guided decoding (GBNF) фиксирует форму.** Грамматика: `verb(outline|query|dot) → опц.target(--line N|
  range) → опц.modifier(force|raw)` + валидные токены на позиции. Модель генерит валидное «предложение»
  — не переставит, не напишет лишнего, полисемия становится пуленепробиваемой (в контексте `--line`
  грамматика разрешает для `--level` только «предок»-значения). Это рычаг «мало слов + невозможно
  ошибиться синтаксисом»; вы уже в llama.cpp (jinja/`tag_tagged`) — механизм под рукой.
